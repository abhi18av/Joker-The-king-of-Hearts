...
module DefiningGenericClasses =
    ...
    type WallBuilder<'T>(initialElement: 'T) = // 'T is the type parameter for the class

        /// Store the states in an array
        let mutable heights = [ initialElement ]

        /// Add a new element to the list of states
        member this.UpdateHeightOfWall newHeightOfWall =
            heights <- newHeightOfWall :: heights  // use the '<-' operator to mutate the value


        /// Get the entire list of historical states
        member this.PreviousHeightsOfWall = heights

 
        /// Get the latest state
        member this.CurrentHeightOfWall = heights.Head


    /// An 'int' instance of the state tracker class. Note that the type parameter is inferred.
    let tracker = WallBuilder 10


    // Add a state
    tracker.UpdateHeightOfWall 17
